  ...
  return (
    <div className="space-y-4 flex flex-col h-[calc(100vh-200px)]">
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pb-2">
        {messages.length === 0 && (
          <div className="text-center py-8 anim-fade-in-up">
            ...
            <div className="space-y-2">
              {suggestions.map((s, i) => (
                <button key={i} onClick={() => send(s)}
                  className="block w-full text-left bg-gray-50 border border-gray-200 rounded-xl px-4 py-2.5 text-sm text-gray-700 hover:border-primary hover:bg-green-50 transition hover:scale-[1.01] anim-fade-in-up" style={{animationDelay: `${i * 60}ms`}}>
                  {s}
                </button>
              ))}
            </div>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-2.5 anim-fade-in-up ${m.role === 'user' ? 'justify-end' : 'justify-start'}`} style={{animationDelay: `${i * 40}ms`}}>
            {m.role === 'assistant' && (
              <div className="w-7 h-7 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                <Bot size={15} className="text-primary" />
              </div>
            )}
            <div className={`max-w-xs rounded-2xl px-4 py-2.5 text-sm leading-relaxed transition hover:scale-[1.01] ${
              m.role === 'user'
      ...
      {lastTrace.length > 0 && (
        <button onClick={() => setShowTrace(!showTrace)}
          className="text-xs text-primary underline text-left transition hover:translate-x-1">
      ...
      {/* Input */}
      <div className="flex gap-2 items-end anim-fade-in-up">
        <input
        ...
        <button onClick={() => send()} disabled={loading || !input.trim()}
          className="w-11 h-11 bg-primary rounded-xl flex items-center justify-center text-white disabled:opacity-40 flex-shrink-0 transition hover:scale-105 active:scale-95">
          <Send size={18} />
        </button>
      </div>
    </div>
