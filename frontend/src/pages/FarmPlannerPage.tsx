...
export default function FarmPlannerPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <section className="card bg-gradient-to-br from-[#eff8ef] via-white to-[#fff8e8] border-primary/15 anim-fade-in-up">
        ...
        <button onClick={generatePlan} className="btn-primary mt-4 inline-flex items-center gap-2 transition hover:scale-105"><Plus size={16} /> Generate plan</button>
      </section>

      {plan ? <>
        <section className="card card-hover anim-fade-in-up stagger-1">
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <button onClick={() => savePlan(null)} className="inline-flex items-center gap-1.5 text-xs font-semibold text-gray-400 hover:text-red-500 transition"><Trash2 size={14} /> Clear plan</button>
          </div>
        ...
        <section className="space-y-3">
          {plan.tasks.map((task, index) => <button key={task.id} onClick={() => toggleTask(task.id)} className={`card w-full text-left flex items-start gap-3 transition-all hover:scale-[1.01] anim-fade-in-up ${task.done ? 'bg-primary/[.04] border-primary/20' : 'hover:border-primary/30'}`} style={{animationDelay: `${index * 70}ms`}}><span className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border ${task.done ? 'border-primary bg-primary text-white' : 'border-gray-300 text-gray-400'}`}>{task.done ? <Check size={15} /> : <span className="text-xs font-bold">{index + 1}</span>}</span><span className="flex-1"><span className={`flex items-center justify-between gap-3 font-semibold ${task.done ? 'text-primary line-through' : 'text-gray-900'}`}><span>{task.title}</span><span className="text-xs font-medium text-gray-400 no-underline">{task.timing}</span></span><span className="mt-1 block text-xs leading-relaxed text-gray-500">{task.detail}</span></span></button>)}
        </section>
      </> : <section className="card py-12 text-center anim-fade-in-up"><ClipboardList size={28} className="mx-auto text-primary/40" /><h2 className="font-semibold mt-3">No active plan yet</h2><p className="text-sm text-gray-500 mt-1">Create a plan to keep field work visible and on time.</p></section>}
