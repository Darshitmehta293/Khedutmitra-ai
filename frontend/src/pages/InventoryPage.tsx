...
export default function InventoryPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <form onSubmit={addItem} className="card card-hover anim-fade-in-up">
      ...
      {loading ? <div className="flex justify-center py-12"><Loader2 className="animate-spin text-primary" /></div> : <>
        <section className="space-y-3 anim-fade-in-up stagger-1">
          <h2 className="font-bold">Active lots</h2>
          {items.length === 0 ? <div className="card text-sm text-gray-500">No inventory lots yet.</div> : items.map(item => <div key={item.id} className="card card-hover flex items-center justify-between gap-3 transition hover:translate-x-1"><div><div className="font-bold">{item.crop.name}</div><div className="text-sm text-gray-500">{item.quantity} quintals · Grade {item.quality_grade} · {item.district || 'District not set'}</div></div><button title="Archive lot" onClick={() => removeItem(item.id)} className="p-2 text-gray-400 hover:text-red-500 transition"><Trash2 size={17} /></button></div>)}
        </section>
        <section className="space-y-3 anim-fade-in-up stagger-2"><h2 className="font-bold">Recommendation history</h2>{recommendations.length === 0 ? <div className="card text-sm text-gray-500">Your saved recommendations will appear here.</div> : recommendations.map(item => <div key={item.id} className="card card-hover transition hover:translate-x-1"><div className="flex justify-between"><strong>{item.action.replace('_', ' ')}</strong><span className="text-xs text-gray-400">{new Date(item.created_at).toLocaleDateString()}</span></div><div className="text-sm text-gray-600 mt-1">Net revenue ₹{item.expected_net_revenue.toLocaleString('en-IN')} · Potential gain ₹{item.potential_gain.toLocaleString('en-IN')}</div><p className="text-sm text-gray-500 mt-2">{item.explanation}</p></div>)}</section>
      </>}
    </div>
  )
}
