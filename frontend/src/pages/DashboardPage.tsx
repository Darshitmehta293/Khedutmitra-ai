...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div className="flex items-start justify-between border-b border-gray-200/80 pb-5 anim-fade-in-up">
        <div>
      ...
      {data?.recommendation_action && (
        <div className="card bg-gradient-to-br from-[#eff8ef] via-white to-[#fff8e8] border-primary/15 anim-fade-in-up stagger-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="card card-hover anim-fade-in-up stagger-1"><StatCard label={t('dashboard.current_value')} value={fmt(data?.current_estimated_value || 0)} sub={`${data?.total_inventory_quintals || 0} quintals`} color="text-primary" /></div>
        <div className="card card-hover anim-fade-in-up stagger-2"><StatCard label={t('dashboard.expected_value')} value={fmt(data?.expected_value_7d || 0)} sub="7-day forecast" /></div>
        <div className="card card-hover anim-fade-in-up stagger-3"><StatCard label={t('dashboard.buyers_available')} value={String(data?.active_buyer_opportunities || 0)} sub="matched buyers" color="text-blue-600" /></div>
      ...
      <div className="card card-hover anim-fade-in-up stagger-2">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-amber-50 rounded-xl p-4 transition hover:scale-[1.01]">
          ...
          <div className="bg-green-50 rounded-xl p-4 transition hover:scale-[1.01]">
        ...
        <div className="card card-hover anim-fade-in-up stagger-4">
          ...
          <div className="space-y-2">
            {data.top_buyers.slice(0, 3).map((b, i) => (
              <div key={i} className="flex items-center justify-between border border-gray-100 rounded-xl px-4 py-2.5 transition hover:border-primary/30 hover:translate-x-1">
                <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link to="/sell-or-store" className="card card-hover anim-fade-in-up stagger-5 cursor-pointer text-center py-4">
          <div className="text-2xl mb-1">⚖️</div>
          <div className="font-semibold text-sm">Sell or Store?</div>
          <div className="text-xs text-gray-400">Full AI analysis</div>
        </Link>
        <Link to="/ai" className="card card-hover anim-fade-in-up stagger-6 cursor-pointer text-center py-4">
